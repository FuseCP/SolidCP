...
namespace SolidCP.WebDav.Core.Owa
{
    public class CobaltHostLockingStore : HostLockingStore
    {
        public override WhoAmIRequest.OutputType HandleWhoAmI(WhoAmIRequest.InputType input)
        {
            ...
            result.UserIsAnonymous = ScpContext.User == null;
            if (ScpContext.User != null)
            {
                result.UserEmailAddress = ScpContext.User.Login;
                result.UserLogin = ScpContext.User.Login;
                result.UserName = ScpContext.User.DisplayName;
            }
