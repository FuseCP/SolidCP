...
namespace SolidCP.Providers.Virtualization
{
	public class ApiClient
	{
		...
		public RestResponse Login(User user)
		{
			...
			apiTicketdata.ticket = data.GetProperty("ticket").GetString();
			apiTicketdata.username = data.GetProperty("username").GetString();
			apiTicketdata.CSRFPreventionToken = data.GetProperty("CSRFPreventionToken").GetString();
		...
		*/
		...
		private RestRequest PrepareGetRequest(string resource)
		{
			...
			request.AddCookie("PVEAuthCookie", apiTicket.ticket, resource, new Uri(baseUrl).Host);
			//request.RootElement = rootElement;
		...
		private ApiVM nodeid(string vmId)
		{
			...
			var jsonResponse = JsonDocument.Parse(RestResponse.Content);
			var jsonResponsearray = jsonResponse.RootElement.GetProperty("data");

			foreach (JsonElement resources in jsonResponsearray.EnumerateArray())
			{
				try
				{
					if (resources.GetProperty("type").GetString().Equals("qemu") && resources.GetProperty("vmid").GetString().Equals(apivm.id))
					{
						apivm.node = resources.GetProperty("node").GetString();
						return apivm;
					}
				}
