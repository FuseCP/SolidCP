...
public partial class PackageVlanConfiguration: Extensions.EntityTypeConfiguration<PackageVlan>
{
    ...
    public override void Configure() {
        ...
        ToTable("PackageVLANs");

        HasIndex(e => e.PackageId, "PackageVLANsIdx_PackageID");

        HasIndex(e => e.VlanId, "PackageVLANsIdx_VlanID");

        Property(e => e.PackageVlanId).HasColumnName("PackageVlanID");
        Property(e => e.PackageId).HasColumnName("PackageID");
        Property(e => e.VlanId).HasColumnName("VlanID");

        HasOne(d => d.Package).WithMany(p => p.PackageVlans)
                .HasForeignKey(d => d.PackageId)
                .HasConstraintName("FK_PackageID");

        HasOne(d => d.Vlan).WithMany(p => p.PackageVlans)
